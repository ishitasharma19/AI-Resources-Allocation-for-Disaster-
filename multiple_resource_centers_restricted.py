# -*- coding: utf-8 -*-
"""Multiple_resource_centers_restricted.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/13AxyHKxSHZiL2GKQhfqnJlHk_TpaeNS5
"""
...
class QLearning:
  ...
  def checkValidity(self,action):
    for idx,center_action in enumerate(action):
      for i,current_action in enumerate(action):
        if idx!=i:
          #make sure the tuples are valid
          for area in range (self.number_of_affected_areas):
            if center_action[area]!=0 and current_action[area]!=0:
              return False
    return True

  def collectValidActions(self):
    new_valid_actions = self.actions.copy()
    for action in self.actions:
      if self.checkValidity(action)==False:
        new_valid_actions.remove(action)
    self.actions = new_valid_actions
...
result = find_path(target_model,env)
